...
export async function fetchListingFromUrl(
  ...
  if (!res.ok) {
    throw new Error(
      `Could not load that listing (HTTP ${res.status}). Check the URL or try again later.`,
    );
  }
  ...
  if (!draft.title || draft.title === "Imported listing") {
    throw new Error(
      "Could not read listing details from that page. The site may be blocking automated access.",
    );
  }
